...
namespace ConsistencyChecks {
        ...
        int map_index_diff)
    {
        ...
        if (point_is_inside_self_box) {
            ...
            if (logger && logger->should_log(spdlog::level::debug)) {
                logger->trace("[MapCheck {}] -> Returning FALSE (Point inside self-box)", case_str);
            }
            return false;
        }
        ...
        if (result.status == PointCloudUtils::InterpolationStatus::SUCCESS) {
            ...
            if (logger && logger->should_log(spdlog::level::debug)) {
                logger->trace("[MapCheck {}] -> Returning {}", case_str, is_consistent);
            }
            ...
            if (logger && logger->should_log(spdlog::level::debug)) {
                logger->trace("[MapCheck {}] -> Returning FALSE (Interpolation Failed)", case_str);
            }
            return false;
        }
    }
        ...
        ConsistencyCheckType check_type)
    {
        ...
        if (stats.static_neighbors_evaluated == 0) {
            ...
            if (logger && logger->should_log(spdlog::level::debug)) {
                logger->trace("[DepthCheck {}] -> Returning FALSE (No suitable STATIC neighbors evaluated)", case_str);
            }
            return false;
        }
        ...
        if (stats.count_close > 0) {
            ...
            if (avg_abs_diff_close > current_depth_threshold) {
                // Apply should_log for debug
                if (logger && logger->should_log(spdlog::level::debug)) {
                    logger->trace("[DepthCheck {}] -> Returning FALSE (Rule 1 Failed: Avg diff > threshold)", case_str);
                }
                return false;
            }
            // Apply should_log for trace
        ...
        if (stats.count_closer == 0 || stats.count_farther == 0) {
            // Apply should_log for debug
            if (logger && logger->should_log(spdlog::level::debug)) {
                logger->trace("[DepthCheck {}] -> Returning TRUE (Rule 2 Passed: No mix of closer/farther)", case_str);
            }
            return true;
        } else {
            // Apply should_log for debug
            if (logger && logger->should_log(spdlog::level::debug)) {
                logger->trace("[DepthCheck {}] -> Returning FALSE (Rule 2 Failed: Mix of closer/farther)", case_str);
            }
            return false;
        }
    }
