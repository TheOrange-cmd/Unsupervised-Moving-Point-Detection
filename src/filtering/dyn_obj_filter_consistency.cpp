...
bool DynObjFilter::checkAppearingPoint(point_soph& p) {
    ...
    if (logger && logger->should_log(spdlog::level::debug)) {
        // Get current sequence ID for context
        uint64_t current_seq_id_for_log_local = PointCloudUtils::g_current_logging_seq_id;
        logger->debug("[checkAppearingPoint] Point OriginalIdx={} (Frame {}): Initial Check. depth_map_list.size()={}, available_maps_to_check={}, min_maps_needed={}",
                      p.original_index, current_seq_id_for_log_local,
                      depth_map_list_.size(), available_maps_to_check, min_maps_needed);
    }
    ...
    // --- CORRECT PLACEMENT FOR THE LOOP ITERATION LOG ---
    const DepthMap& map = *depth_map_list_[i]; // Define map first!
    if (logger) { // Keep should_log_point_details for this one as it's per iteration
        uint64_t current_seq_id_for_log_local = PointCloudUtils::g_current_logging_seq_id;
        logger->debug("[checkAppearingPoint] Point OriginalIdx={} (Frame {}): Loop Iteration for MapIdx={}. GeoCnt={}, EmptyCnt={}, GeoThr1={}, GeoThr2={}, RemainingMapChecksToDo={}",
                      p.original_index, current_seq_id_for_log_local, map.map_index,
                      geometric_inconsistent_count, empty_neighbor_inconsistent_count,
                      params_.occluded_map_thr1, params_.occluded_map_thr2, i + 1); // i+1 is how many maps are left to check in this loop
    }
    // --- END CORRECT PLACEMENT ---
        ...
        // --- ADD THIS LOG ---
        if (logger && PointCloudUtils::should_log_point_details(logger)) { // Or just logger->should_log(spdlog::level::debug)
            logger->debug("[checkAppearingPoint] Point {}: Before Early Exit Check: can_reach_geometric_thr={}, can_reach_empty_thr={}",
                        p.original_index, can_reach_geometric_thr, can_reach_empty_thr);
        }
        // --- END ADDED LOG ---
