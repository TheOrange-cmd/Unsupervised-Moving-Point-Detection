...
def calculate_adaptive_epsilon(
    ...
    if not enabled: # Simplified d_anchor check for Numba
        return di_base
    if d_anchor is None: # Check for None explicitly
        return di_base
    if d_anchor < 0:
        return di_base
    ...
    # Clip result
    if calculated_epsilon < dmin:
        final_epsilon = dmin
    elif calculated_epsilon > dmax:
        final_epsilon = dmax
    else:
        final_epsilon = calculated_epsilon

    return float(final_epsilon) # Ensure it returns a standard Python float if needed outside Numba