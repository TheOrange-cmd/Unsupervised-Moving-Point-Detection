...
def compose_gt_vs_mdet_frame(
    ...
    video_gen_cfg = viz_params['video_generation']
    gt_style_cfg = viz_params['ground_truth_style']
    mdet_style_cfg = viz_params['mdetector_style']
    common_bev_cfg = viz_params['common_bev_style']
    ...
    if mdet_min_range_override is not None and mdet_max_range_override is not None:
        ...
        mdet_min_range_to_use = point_pre_filtering_params['min_range_meters']
        mdet_max_range_to_use = point_pre_filtering_params['max_range_meters']
    ...
    gt_vel_thresh = validation_params['gt_velocity_threshold']
    ...
    fig.suptitle(fig_title_template.format(vel_thresh=gt_vel_thresh, frame_num=frame_number_in_video), 
                 fontsize=video_gen_cfg['figure_title_fontsize'])
    ...
    try:
        ...
        if gt_style_cfg['show_static_points']:
           gt_points_to_plot['GT_Static_Labeled'] = gt_point_categories.get('static', np.empty((0,3)))
        if gt_style_cfg['show_unlabeled_points_as_background_detail']:
        ...
        gt_plot_specific_cfg = {**common_bev_cfg, **gt_style_cfg['subplot_style']}
        ...
        mdet_plot_specific_cfg = {**common_bev_cfg, **mdet_style_cfg['subplot_style']}
        ...
        fig.tight_layout(rect=[0, 0.03, 1, video_gen_cfg['tight_layout_rect_top_factor']])
...
#         ax = axes[i]
#         # subplot_title = f"Run: {mdet_result_this_run['source_name']}"
