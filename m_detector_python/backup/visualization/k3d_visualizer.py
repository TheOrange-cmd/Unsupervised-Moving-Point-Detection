...
from ..utils.label_generation import get_interpolated_extrapolated_boxes_for_instance, find_instances_in_scene 
...
def visualize_sweep_k3d(
    ...
    plot = k3d.plot(name=f"Scene: {nusc.get('scene', scene_token)['name']} - Sweep: {target_sweep_lidar_sd_token[:8]}",
                    grid_visible=k3d_plot_cfg['grid_visible'],
                    camera_auto_fit=k3d_plot_cfg['camera_auto_fit'],
                    background_color=k3d_plot_cfg['plot_background_color_hex'])


    bg_points_to_plot = raw_points_global[::downsample_factor]
    plot += k3d.points(positions=bg_points_to_plot.astype(np.float32),
                       color=k3d_plot_cfg['background_points_color_hex'],
                       point_size=point_size * k3d_plot_cfg['background_points_scale_factor'],
                ...
                if gt_point_labels_for_sweep is not None and \
                   gt_point_labels_for_sweep.shape[0] == raw_points_global.shape[0]:
                    ...
                    if config_accessor:
                        ...
                        vel_thresh_for_gt_dynamic = validation_p['gt_velocity_threshold']
                    ...
                    if gt_dynamic_pts.shape[0] > 0:
                        plot += k3d.points(positions=gt_dynamic_pts.astype(np.float32),
                                           color=k3d_plot_cfg['gt_dynamic_points_color_hex'],
                                           point_size=point_size * k3d_plot_cfg['gt_dynamic_points_scale_factor'], 
                    ...
                    if gt_static_pts.shape[0] > 0:
                         plot += k3d.points(positions=gt_static_pts.astype(np.float32),
                                           color=k3d_plot_cfg['gt_static_points_color_hex'],
                                           point_size=point_size * k3d_plot_cfg['gt_static_points_scale_factor'],
        ...
        try:
            with h5py.File(mdet_results_hdf5_path, 'r') as mdet_h5:
                ...
                if mdet_all_preds_for_sweep is not None:
                    mdet_dynamic_val = k3d_plot_cfg['mdet_dynamic_label_value']
                    mdet_occluded_val = k3d_plot_cfg['mdet_occluded_label_value']
                    mdet_undetermined_val = k3d_plot_cfg['mdet_undetermined_label_value']
                    ...
                    if mdet_dyn_pts_global.shape[0] > 0:
                        plot += k3d.points(positions=mdet_dyn_pts_global.astype(np.float32),
                                           color=k3d_plot_cfg['mdet_dynamic_points_color_hex'],
                                           point_size=point_size * k3d_plot_cfg['mdet_dynamic_points_scale_factor'], 
                    ...
                    if mdet_occ_pts_global.shape[0] > 0:
                        plot += k3d.points(positions=mdet_occ_pts_global.astype(np.float32),
                                           color=k3d_plot_cfg['mdet_occluded_points_color_hex'],
                                           point_size=point_size * k3d_plot_cfg['mdet_occluded_points_scale_factor'], 
                    ...
                    if mdet_und_pts_global.shape[0] > 0 and k3d_plot_cfg['show_mdet_undetermined_points']:
                        plot += k3d.points(positions=mdet_und_pts_global.astype(np.float32),
                                           color=k3d_plot_cfg['mdet_undetermined_points_color_hex'],
                                           point_size=point_size * k3d_plot_cfg['mdet_undetermined_points_scale_factor'],
    ...
    if show_gt_boxes:
        ...
        if current_sweep_idx_in_list != -1:
            num_instance_colors = k3d_plot_cfg['num_instance_box_colors']
            instance_colormap_name = k3d_plot_cfg['instance_box_colormap']
                ...
                if gt_box_at_sweep:
                    ...
                    for start_idx, end_idx in box_edges:
                        ...
                        plot += k3d.line(segment_vertices, shader='simple', 
                                         ...
                                         width=k3d_plot_cfg['gt_box_line_width'],
