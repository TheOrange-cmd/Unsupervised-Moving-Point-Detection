...
from ..utils.label_generation import get_interpolated_extrapolated_boxes_for_instance
...
from .transformations import transform_points_numpy   # For transforming points to global
    ...
    for token, data in instance_animation_data.items():
        ...
        artists_dict['legend_elements'].append(
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor=color, markersize=10, 
                       label=data['category']) # Use .get for safety
        )
